...
export default function PendingMonitoringTable({
  data = { columns: [], rows: [] },
  sortData = () => {},
}) {
    console.log('data in pending', data)
  return (
    <table className="w-full rounded-t-lg overflow-hidden">
      <tr className="bg-BGLight">
        {data?.columns?.map((column, index) => (
          <th
            ...
            className={`py-3 px-6 min-w-150 ${column.sortAllowed ? 'cursor-pointer' : ''}`}
            onClick={() => {
          ...
          >
            ...
            >
                ...
                {
                    column.sortAllowed && column.sortDirection === 'descending' && (
                        <FaSortDown
                            className="text-secondary"
                        />
                    )
      ...
      </tr>
      {
        data?.rows?.map((row, index) => (
            <tr className="border-solid border-BGBorder"
                key={`row-${index}`}
                style={{
                    borderWidth: '1px'
                }}
            >
                ...
                >
                    {
                        row.previouslyReviewed.reviewd ? (
                            <>
                                <h6>
                                    Yes
                                </h6>
                                <h6
                                    className="text-secondary"
                                >
                                    {moment(row.previouslyReviewed.date).format('DD MMM, YYYY')}
                                </h6>
                            </>
                        ) : (
                            <h6>
